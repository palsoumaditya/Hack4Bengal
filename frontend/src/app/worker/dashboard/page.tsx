...
import { FiUser, FiLogOut, FiDollarSign, FiClock, FiCheckCircle, FiMapPin, FiBell, FiTrendingUp, FiList, FiTarget, FiNavigation, FiBriefcase, FiX, FiRadio, FiAlertTriangle, FiEdit, FiSave, FiActivity, FiPower, FiArchive, FiXCircle } from 'react-icons/fi';
...
const WeeklyLineChart = () => {
    const options = {
        ...
        scales: {
            ...
            y: { grid: { color: 'rgba(200, 200, 200, 0.1)', borderDash: [5, 5] }, beginAtZero: true }
        },
    ...
    const data = {
        ...
        datasets: [
            {
                fill: true, label: 'Earnings ($)', data: [180, 400, 290, 350, 80, 500, 270],
                borderColor: 'rgb(22, 163, 74)', backgroundColor: 'rgba(22, 163, 74, 0.2)', tension: 0.3,
            },
            {
                fill: true, label: 'Jobs Completed', data: [3, 8, 5, 6, 2, 9, 5],
                borderColor: 'rgb(59, 130, 246)', backgroundColor: 'rgba(59, 130, 246, 0.2)', tension: 0.3,
            },
        ],
    };
    return <div className={styles.lineChartContainer}><Line options={options} data={data} /></div>;
};

// --- MODIFIED: Updated Profile Type Definition ---
type Profile = {
    firstName: string;
...
export default function WorkerDashboardPage() {
  ...
  // --- MODIFIED: Profile state is now updatable with a default value ---
  const [profile, setProfile] = useState<Profile>({ firstName: 'Worker', imageUrl: null });
        ...
        <div className={styles.dashboardContainer}>
          <header className={styles.header}>
            ...
            <div className={styles.headerActions}>
              ...
              {/* --- MODIFIED: Profile block now displays dynamic data and links to edit page --- */}
              <div className={styles.profileBlock}>
                <button className={styles.iconButton} onClick={() => router.push('/worker/onboarding')}>
                    {profile.imageUrl ? (
                        <img src={profile.imageUrl} alt="Profile" className={styles.profilePicture} />
                    ) : (
                        <FiUser />
                    )}
                </button>
          ...
          <main className={styles.contentGrid}>
            ...
            <div className={styles.rightSidebar}>
                <div className={styles.statCardRow}>
                    ...
                    <div className={`${styles.card} ${styles.statCard}`}>
                        ...
                        <div className={styles.statTextContainer}>
                            <div className={styles.statValue}>{formatTime(timeWorked)}</div>
                ...
                <div className={`${styles.card} ${styles.chartCard}`}>
                    <h3 className={styles.cardHeader}><FiActivity/> Weekly Activity</h3>
                    <WeeklyLineChart />
                </div>
            </div>
              ...
              {isJobAccepted && jobRequest ? (
                <div className={styles.activeJobContent}>
                  ...
                  <div className={styles.activeJobRow}>
                      ...
                      <span className={styles.activeJobValue}>{jobRequest.title}</span>
                  </div>
                  <div className={styles.activeJobRow}>
                      <span className={styles.activeJobLabel}>Est. Fare</span>
                      <span className={styles.activeJobValue}>${jobRequest.fare.toFixed(2)}</span>
                  </div>
                  <button className={`${styles.jobButton} ${styles.completeButton}`} onClick={handleCompleteJob}>
                      Complete Job
                  </button>
                </div>
              ) : (
                <div className={styles.emptyStateContainer}>
                    ...
                    <h4 className={styles.emptyStateTitle}>{isLive ? 'Ready for Jobs' : 'You Are Offline'}</h4>
                    <p className={styles.emptyStateText}>
                        {isLive ? 'Waiting for the next available job in your area.' : 'Go live to start receiving job alerts from clients.'}
                    </p>
                    {!isLive && (
                        <button className={`${styles.jobButton} ${styles.goLiveCardButton}`} onClick={() => setIsLive(true)}>
                            <FiRadio /> Go Live
                        </button>
                    )}
                </div>
              ...
              <h3 className={styles.cardHeader}><FiList /> Job History</h3>
                {jobHistory.length > 0 ? (
                        ...
                        {jobHistory.map(job => (
                            <div className={styles.jobItem} key={job.id}>
                                ...
                                <div className={styles.jobItemInfo}>
                                    ...
                                    <span className={styles.jobItemCompany}>Distance: {job.distance} km</span>
                                </div>
                    ...
                    <div className={styles.emptyStateContainer}>
                        ...
                        <p className={styles.emptyStateText}>Your completed and declined jobs will appear here.</p>
                    </div>
                )}
            </div>
            ...
            <div className={`${styles.card} ${styles.performanceCard}`}>
              ...
              <div className={styles.performanceContent}>
                <div><div className={styles.circularProgress} style={{'--v': `${performance.rating / 5 * 100}%` } as React.CSSProperties}><span className={styles.progressValue}>{performance.rating}</span></div><p className={styles.performanceLabel}>Avg. Rating</p></div>
                <div><div className={`${styles.circularProgress} ${styles.green}`} style={{'--v': `${performance.successRate}%` } as React.CSSProperties}><span className={styles.progressValue}>{performance.successRate}%</span></div><p className={styles.performanceLabel}>Success Rate</p></div>
              </div>
            </div>
            ...
            <div className={`${styles.card} ${styles.goalCard}`}>
                <div className={styles.goalHeader}>
                    ...
                    <button className={styles.editGoalButton} onClick={() => setIsEditingGoal(!isEditingGoal)}>
                ...
                {isEditingGoal ? (
                    <div className={styles.goalEditContainer}>
                        ...
                        />
                        <button className={styles.goalSetButton} onClick={handleSetGoal}><FiSave/> Set</button>
                    </div>
                ) : (
                    <div className={styles.goalDisplay}>
                        <div
                            className={styles.goalCircularProgress}
                            style={{ '--progress': `${Math.min(100, (earnings / weeklyGoal.target) * 100)}%` } as React.CSSProperties}
                        >
                          <div className={styles.goalInnerCircle}>
                              <span className={styles.goalCurrentValue}>${earnings.toFixed(2)}</span>
                        ...
                        <div className={styles.goalTargetText}>
                            Target: <span className={styles.goalTargetValue}>${weeklyGoal.target.toLocaleString()}</span>
                        </div>
                    </div>
                )}
            </div>
